...
bool WindMill::predict(const armorData data,Point2f& preCenter,int pMode)
{
    ...
    if(Max_Motion_Predict(data) == 0 && shoot_mode == SHOOT_MAX) return false;
...
void WindMill::detect(const Mat frame,int Mode,Point2f &pt, Point2f pts[4], int &status){
    ...
    if(shoot_mode==SHOOT_MIN)
    {
        ...
        if(getArmorCenter(src,armordata) == false){
            ...
            if(predict(armordata,preCenter,param.pMode) == false){
                ...
                for(int i=0;i<4;i++){
                    if(pre_points[i]!=Point2f(0,0))
                        pts[i]=pre_points[i];
                }

#ifdef SHOW_PRECENTER
                for (int i = 0; i < 4; i++)
                    line(src, pts[i], pts[(i+1)%4], Scalar(0,255,0),3);
