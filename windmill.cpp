            ...
            if (contourArea(contours[i])> param.Leaf_minArea)
            {
                ...
                if(svm->predict(svm_mat) ==1 && (hierarchy[i][2] > 0))
                {
                    if (hierarchy[hierarchy[i][2]][1] < 0 && hierarchy[hierarchy[i][2]][0] < 0)
                    {
                        ...
                        circle(src,arrowCenter,1,Scalar(0,0,255),8,LINE_8,0);
                        imshow("aim",src);
#ifdef SINGAL
                        waitKey(0);
#endif
...
bool WindMill::predict(const armorData data,Point2f& preCenter,int pMode){
    ...
    if(pMode == FIT_CIRCLE)
    {
        ...
        if(count < param.infer_frame_number){
            ...
            // 二维旋转
            float preAngle;
            if(shoot_mode==SHOOT_MIN) preAngle=2 * CV_PI * param.Min_n * param.shoot_time;
            else preAngle=Max_Motion_Predict(data);

            if(detect_mode == RED_ANCLOCK || detect_mode == BLUE_ANCLOCK){// 逆时针
...
//            }
    ...
    {// 先预测到切线方向，然后在切线方向上进行旋转的补偿
       ...
       Mat draw_show=Mat::zeros(src.size(),CV_8UC3);
       circle(draw_show,preCenter,5,Scalar(255,0,0),2);
       circle(draw_show,data.armorCenter,5,Scalar(0,0,255),2);
       imshow("precenter",draw_show);
#ifdef SINGAL
       waitKey(0);
#endif
#endif
...
double WindMill::Max_Motion_Predict(const armorData new_data){
    ...
    do
    {
        ...
        ws.push_back(w);
        if(ws.size()>param.infer_frame_number-5){
...
void WindMill::detect(const Mat frame,int Mode,Point2f &pt, int &status){
    ...
    {//detect_mode == 7 || detect_mode == 8){// 小符
        armorData armordata;
    ...
    {//detect_mode == 3 || detect_mode == 4 || detect_mode == 5 || detect_mode == 6){// 大符
        armorData armordata;
        if(getArmorCenter(src,armordata) == false){
            pt = Point2f(0,0);
        }
